...
from intent_rule import intent_classify_rule
...
if __name__ == '__main__':
    ...
    for content in content_list:
        if intent_classify_rule(content[0]):
            in_rule_pre.append(1)
            in_rule_true.append(content[1])
        else:
            out_rule.append(content)
    content_list = out_rule
    ...
    for index in range(len(contentid_match_list)):
        ...
        test_loss, _, test_predictions, test_real = model.dev_step([batch_test_x, batch_test_y], real_test_length)
        test_predictions = list(test_predictions)
        test_real = list(test_real)
        test_predictions.extend(in_rule_pre)
        test_real.extend(in_rule_true)
        test_accuracy = accuracy_score(test_predictions, test_real)
