...
class IntentModel(object):
    ...
    def __init__(self):
        ...
        self.rnn_length = tf.placeholder(tf.int32, shape=[None], name='rnn_length')
        ...
        with tf.name_scope('LSTM'):
            ...
            Cell = tf.contrib.rnn.DropoutWrapper(cell, self.dropout_keep_prob)
            LSTM_output, _ = tf.nn.dynamic_rnn(cell=Cell, inputs=self.embedded_chars, sequence_length=self.rnn_length,
                                          dtype=tf.float32)

        # Create a convolution + maxpool layer for each filter size
        pooled_outputs = []
        self.embedded_chars_expanded = tf.expand_dims(LSTM_output, -1)
    ...
    def train_step(self, batch, rnn_length):
        """
    ...
    def dev_step(self, batch, rnn_length):
        """
    ...
    def test_step(self, batch, rnn_length):
        """
        Evaluates model on a dev set
        """
        feed_dict = {
